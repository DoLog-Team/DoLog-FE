import type { ExhibitionItem } from "@/lib/api/exhibition";

export const MOCK_EXHIBITIONS: ExhibitionItem[] = [
    {
        id: "1",
        slug: "mock-1",
        title: "흙에서 시작되는 모든 이야기",
        univName: "한국대학교",
        deptName: "도예과",
        address: "한국대학교 문화관 지하1층 아트홀",
        imageUrl: "/images/cups.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-29",
        dday: null,
    },
    {
        id: "2",
        slug: "mock-2",
        title: "되돌아가는 삶",
        univName: "한국대학교",
        deptName: "불교미술전공",
        address: "한국대학교 예술관 지하1층 101호",
        imageUrl: "/images/plate.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-29",
        dday: null,
    },
    {
        id: "3",
        slug: "mock-3",
        title: "마음을 여닫는 힘",
        univName: "한국대학교",
        deptName: "시각디자인학과",
        address: "한국대학교 문화관 3층 대강당",
        imageUrl: "/images/pottery.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-29",
        dday: null,
    },
    {
        id: "4",
        slug: "mock-4",
        title: "비대칭과 균형",
        univName: "한국대학교",
        deptName: "조소",
        address: "한국대학교 문화관 지하1층 아트홀",
        imageUrl: "/images/pottery.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-28",
        dday: null,
    },
    {
        id: "5",
        slug: "mock-5",
        title: "빛",
        univName: "서울예술대학교",
        deptName: "서양화전공",
        address: "서울예술대학교 미술관 지하1층 102호",
        imageUrl: "/images/plate.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-29",
        dday: null,
    },
    {
        id: "6",
        slug: "mock-6",
        title: "마음을 여닫는 형",
        univName: "서울예술대학교",
        deptName: "시각디자인학과",
        address: "서울예술대학교 문화관 3층 대강당",
        imageUrl: "/images/cups.png",
        logoImg: "/images/logo-mock.png",
        startDate: "2025-11-28",
        endDate: "2025-11-29",
        dday: null,
    },
];