...
export function ArtistCard({ author }: ArtistCardProps) {  // artist → author
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-col pb-5">
                ...
                {author.sns.instagram && <RowList rows={[{ label: "instagram", value: author.sns.instagram }]} />}
                {author.sns.x && <RowList rows={[{ label: "X", value: author.sns.x }]} />}
            </div>

            {/* 프로필 더보기 버튼 */}
            <Button variant="outline" size="sm" className="w-full">프로필 더보기</Button>
        </div>
    );
}