...
export function useArtworkFilter(artworks: ArtworkListItem[], searchQuery: string = "") {
	...
	const currentArtworks = (artworks && artworks.length > 0) 
        ? artworks 
        : MOCK_ARTWORK_LIST.artworks; // TODO : 현재 백 데이터가 비어있어서 임시 처리
	console.log("지금 데이터는?", currentArtworks === MOCK_ARTWORK_LIST.artworks ? "가짜(목)" : "진짜(백엔드)");
